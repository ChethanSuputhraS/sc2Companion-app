...
- (void)drawRect:(CGRect)rect {
    ...
    descriptionLabel.textAlignment = NSTextAlignmentCenter;//NSTextAlignmentLeft
    ...
    if (alertTextFields.count > 0) {
        ...
        for (int i = 0; i < MIN(alertTextFields.count,4); i++) {
            ...
            if ([[alertTextFields objectAtIndex:i] objectForKey:@"field"] != nil &&
                ...
                if (tf.tag >= 9000)
                {
                    isSpecialTextfield9000 = YES;
                }
                else
                {
                    isSpecialTextfield9000 = NO;
                }
...
{
    if (isSpecialTextfield9000 == YES)
    {
        if ([string isEqualToString:@""])
        {
            return YES;
        }
        NSString * strCurrentValue = [NSString stringWithFormat:@"%@%@",textField.text,string];
        int intVal = [strCurrentValue intValue];
